...
class Categories {
    ...
    private $_category_parent = 0;
    private $_category_selectable = 0;
    private $_category_has_children = 0;
    ...
    private function _load_set() {
        $category_parent = 0;
        $sql = "SELECT * FROM " . $this->tables['categories'] . " WHERE category_parent = $category_parent";
        //echo $sql . "<br />";
        ...
        while($row = $this->db->sql_fetchrow($result)) {
            $this->values[$row['category_id']] = $row;
            //echo $row['category_id'] . ' - ' . $row['category_has_children'] . '<br />';
            if($row['category_has_children'] == 1) {
                $this->values[$row['category_id']]['children'] = $this->_process_children($row['category_id']);
            } 
        }
    ...
    private function _load_document_from_db($document_id) {
        "SELECT * FROM {$this->tables['categories']} INNER JOIN";
    }
    
    private function _clear_data() {
        $this->_category_id = 0;
        $this->_category_title = "";
        $this->_category_parent = 0;
        $this->_category_selectable = 0;
        $this->_category_has_children = 0;
    }
}
